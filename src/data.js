export const socialLinks = [
  { id: 1, href: 'http://facebook.com', icon: 'fa-facebook' },
  { id: 2, href: 'http://twitter.com', icon: 'fa-twitter' },
  { id: 3, href: 'http://instagram.com', icon: 'fa-instagram' },
  { id: 4, href: 'http://youtube.com', icon: 'fa-youtube' },
]

export const menuItems = [
  { id: 1, href: '/', label: 'Home', isActive: true },
  { id: 2, href: '/about', label: 'About' },
]

export const articlesWithTopImage = [
  {
    id: 1,
    img: 'img/articles/ent1.jpg',
    category: 'Entertainment',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
  },
  {
    id: 2,
    img: 'img/articles/tech1.jpg',
    category: 'Technology',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
  },
  {
    id: 3,
    img: 'img/articles/tech2.jpg',
    category: 'Technology',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
  },
]

export const articlesWithBottomImage = [
  {
    id: 4,
    category: 'Sports',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    img: 'img/articles/sports1.jpg',
  },
  {
    id: 5,
    category: 'Entertainment',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    img: 'img/articles/ent2.jpg',
  },
]

export const articlesWithoutImage = [
  {
    id: 6,
    category: 'Sports',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    bgClass: 'bg-dark',
  },
  {
    id: 7,
    category: 'Sports',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    bgClass: 'bg-primary',
  },
  {
    id: 8,
    category: 'Entertainment',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
  },
]

export const siteLinks = [
  {
    id: 1,
    text: 'Help & Support',
  },
  {
    id: 2,
    text: 'Privacy Policy',
  },
  {
    id: 3,
    text: 'About Us',
  },
  {
    id: 4,
    text: 'Contact',
  },
]

export const articlesData = [
  {
    id: 1,
    img: 'img/articles/ent1.jpg',
    category: 'Entertainment',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    order: ['img', 'content'],
  },
  {
    id: 2,
    category: 'Sports',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    bgClass: 'bg-dark',
    order: ['content'],
  },
  {
    id: 3,
    img: 'img/articles/tech1.jpg',
    category: 'Technology',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    order: ['img', 'content'],
  },
  {
    id: 4,
    img: 'img/articles/sports1.jpg',
    category: 'Sports',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    order: ['content', 'img'],
  },
  {
    id: 5,
    img: 'img/articles/tech2.jpg',
    category: 'Technology',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    order: ['img', 'content'],
  },
  {
    id: 6,
    category: 'Sports',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    bgClass: 'bg-primary',
    order: ['content'],
  },
  {
    id: 7,
    img: 'img/articles/ent2.jpg',
    category: 'Entertainment',
    title: 'Lorem ipsum dolor sit amet',
    description:
      'Lorem ipsum dolor sit amet consectetur adipisicing elit. Nulla quam eius ducimus optio veniam sit nihil beatae ea autem. Doloribus.',
    order: ['content', 'img'],
  },
]

export const list = [
  { id: 1, name: 'Sports' },
  { id: 2, name: 'Entertainment' },
  { id: 3, name: 'Technology' },
  { id: 4, name: 'Fashion' },
  { id: 5, name: 'Shopping' },
]
